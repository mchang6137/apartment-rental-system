...
function nodeServer(count, nodeRepo) {
	...
	this.postgres.placeOn({diskSize: diskSizes[5]});

	this.mysql.placeOn({diskSize: diskSizes[6]});

	this.logstash.placeOn({diskSize: diskSizes[7]});

	this.kib.placeOn({diskSize: diskSizes[8]});
	this.kib1.placeOn({diskSize: diskSizes[9]});
	this.kib2.placeOn({diskSize: diskSizes[10]});
	this.kib3.placeOn({diskSize: diskSizes[11]});
	this.kib4.placeOn({diskSize: diskSizes[12]});
	this.kib5.placeOn({diskSize: diskSizes[13]});

	this.spark.placeOn([diskSizes[14], diskSizes[15], diskSizes[16], diskSizes[17], diskSizes[18], diskSizes[19]]);
    };
